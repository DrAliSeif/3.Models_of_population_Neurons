function wb_init=wb_init(i_ext,phi_vec)
...
while sum(done)<num && t<t_final,
    ...
    for i=1:l,
        k=ind(i);
        wb_init(k,1)=(v_old(k)*(t-thr(k))+v(k)*(thr(k)-t_old))/dt;
        wb_init(k,2)=(h_old(k)*(t-thr(k))+h(k)*(thr(k)-t_old))/dt;
        wb_init(k,3)=(n_old(k)*(t-thr(k))+n(k)*(thr(k)-t_old))/dt;
    end;
    done(ind)=1;
