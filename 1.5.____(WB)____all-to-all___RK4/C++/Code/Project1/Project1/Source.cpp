...
double rk4thOrder_v(double t0, double v, double dt, double n, double h, double m, double g_iis_i, double I_i, double v_rev_i) {
    double  k1, k2, k3, k4;
    k1 = dt * dvdt_i(t0, v, n, h, m,g_iis_i, I_i, v_rev_i);
    k2 = dt * dvdt_i((t0 + dt / 2), (v + k1 / 2), n, h, m, g_iis_i, I_i, v_rev_i);
    k3 = dt * dvdt_i((t0 + dt / 2), (v + k2 / 2), n, h, m, g_iis_i, I_i, v_rev_i);
    k4 = dt * dvdt_i((t0 + dt), (v + k3), n, h, m, g_iis_i, I_i, v_rev_i);
...
double rk4thOrder_n(double t0, double v, double dt, double n) {
    ...
    double  k1, k2, k3, k4;
    k1 = dt * dndt_i(t0, n, v);
    k2 = dt * dndt_i((t0 + dt / 2), (n + k1 / 2), v);
    k3 = dt * dndt_i((t0 + dt / 2), (n + k2 / 2), v);
    k4 = dt * dndt_i((t0 + dt), (n + k3), v);
...
double rk4thOrder_h(double t0, double v, double dt, double h) {
    ...
    double  k1, k2, k3, k4;
    k1 = dt * dhdt_i(t0, h, v);
    k2 = dt * dhdt_i((t0 + dt / 2), (h + k1 / 2), v);
    k3 = dt * dhdt_i((t0 + dt / 2), (h + k2 / 2), v);
    k4 = dt * dhdt_i((t0 + dt), (h + k3), v);
...
double rk4thOrder_q(double t0, double v, double dt, double q, double tau_dq_i) {

    double  k1, k2, k3, k4;
    k1 = dt * dqdt_i(t0, q, v, tau_dq_i);
    k2 = dt * dqdt_i((t0 + dt / 2), (q + k1 / 2), v,tau_dq_i);
    k3 = dt * dqdt_i((t0 + dt / 2), (q + k2 / 2), v,tau_dq_i);
    k4 = dt * dqdt_i((t0 + dt), (q + k3), v, tau_dq_i);
    q = q + double((1.0 / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4));
    return   q;
}

double rk4thOrder_s(double t0, double v, double dt, double q, double s, double tau_r_i,double tau_d_i) {

    double  k1, k2, k3, k4;
    k1 = dt * dsdt_i(t0, s,q ,v, tau_r_i, tau_d_i);
    k2 = dt * dsdt_i((t0 + dt / 2), (s + k1 / 2), q, v, tau_r_i, tau_d_i);
    k3 = dt * dsdt_i((t0 + dt / 2), (s + k2 / 2), q, v, tau_r_i, tau_d_i);
    k4 = dt * dsdt_i((t0 + dt), (s + k3), q, v, tau_r_i, tau_d_i);
...
//__________________________________________________________________solve the system of Hodgkin-Huxley-like equations using the midpoint method
        ...
        //__________________________________________________________________solve the system of Hodgkin-Huxley-like equations using the midpoint method
        for (int i = 0; i < N; i++) {
            v_init[i] = rk4thOrder_v(t0_init, v_init[i], dt, n_init[i], h_init[i], m_init[i], 0, I_app[i], v_rev_i);
            n_init[i] = rk4thOrder_n(t0_init, v_init[i], dt, n_init[i]);
            h_init[i] = rk4thOrder_h(t0_init, v_init[i], dt, h_init[i]);
            m_init[i] = m_inf_i(v_init[i]);
...
//__________________________________________________________________solve the system of Hodgkin-Huxley-like equations using the midpoint method
       ...
       /* double gs;
        ...
        for (int i = 0; i < N; i++) {
            ...
            v[i] = rk4thOrder_v(t0, v[i], dt, n[i], h[i], m[i], gs, I_app[i], v_rev_i);
            n[i] = rk4thOrder_n(t0, v[i], dt, n[i]);
            h[i] = rk4thOrder_h(t0, v[i], dt, h[i]);
            m[i] = m_inf_i(v[i]);
            q[i] = rk4thOrder_q(t0, v[i], dt, q[i], tau_dq_i);
            s[i] = rk4thOrder_s(t0, v[i], dt, q[i], s[i],  tau_r_i, tau_d_i);
            test << '\t' << v[i];
        }
        test << endl;
